...
public class FMODDialogueView : DialoguePresenterBase
{
    ...
    public override YarnTask RunLineAsync(LocalizedLine dialogueLine, LineCancellationToken cancellationToken)
    {
        ...
        waitCoroutine = StartCoroutine(WaitUntilFinished(tcs, cancellationToken));
        return tcs.Task;
    }
    ...
    private IEnumerator WaitUntilFinished(YarnTaskCompletionSource tcs, LineCancellationToken cancellationToken)
    {
        while (instance.isValid())
        {
            // IMPORTANT: do NOT cancel the YarnTask. Just stop audio and complete normally.
            if (cancellationToken.IsNextLineRequested)
            {
                if (log) Debug.Log("[FMODDialogueView] Next line requested: stopping VO and completing task.");
                StopPlayback(allowFadeOut: false);
                tcs.TrySetResult();   // ✅ complete normally, no OperationCanceledException
                yield break;
            }
    ...
    private void OnDisable() => StopPlayback(allowFadeOut: false);
    private void OnDestroy() => StopPlayback(allowFadeOut: false);
}
